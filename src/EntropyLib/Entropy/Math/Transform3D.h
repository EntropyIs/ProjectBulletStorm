...
namespace Entropy
{
	namespace Math
	{
		...
		// Projections
		Mat4 __declspec(dllexport) Ortho(float left, float right, float top, float bottom, float oNear, float oFar);
		Mat4 __declspec(dllexport) Perspective(float fov, float aspectRatio, float pNear, float pFar);
